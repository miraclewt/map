...
%
%   R = MAPREFPOSTINGS(xWorldLimits,yWorldLimits,rasterSize) constructs a
%   referencing object for a raster spanning the specified limits in planar
%   coordinates, with the numbers of rows and columns specified by
%   rasterSize.
%
%   R = MAPREFPOSTINGS(xWorldLimits,yWorldLimits,sampleSpacingInWorldX,sampleSpacingInWorldY)
%   allows the sample spacings to be set precisely. The limits of the
%   raster will be adjusted slightly, if necessary, to ensure an integer
%   number of samples in each dimenstion.
%
%   R = MAPREFPOSTINGS(xWorldLimits,yWorldLimits,___,Name,Value) allows the
