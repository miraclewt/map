...
lat = intrinsicYToLatitude(R,y);
lon = intrinsicXToLongitude(R,x);

% Equivalent to [lonmesh,latmesh] = meshgrid(lon,lat);
latmesh = lat(:,ones(1,ncols));
lonmesh = lon(ones(nrows,1),:);
